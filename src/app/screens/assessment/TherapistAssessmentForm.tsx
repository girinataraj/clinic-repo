  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-950 font-sans overflow-hidden">
      ...
      <div className={`px-5 shrink-0 transition-all duration-300 ${isHeaderExpanded ? 'pt-5 pb-4 rounded-b-[2rem]' : 'py-3.5 rounded-b-2xl'} bg-gradient-to-br from-[#134e4a] to-[#0d9488] dark:from-slate-900 dark:to-slate-800 shadow-lg shadow-teal-900/10 z-10 relative overflow-hidden`}>
        ...
        {isHeaderExpanded && (
          <>
            <div className="rounded-full h-1.5 bg-black/10 dark:bg-white/10 relative z-10 overflow-hidden mb-1">
              <div className="h-full bg-white transition-all duration-700 ease-out rounded-full shadow-[0_0_10px_rgba(255,255,255,0.4)]" style={{width:`${((step+1)/totalSteps)*100}%`}} />
            </div>
            {/* Step Indicators */}
        ...
        {showNewPatientForm&&(
          <div className="mt-3 p-5 rounded-[22px] bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 flex flex-col gap-4 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <input placeholder="Referred By (e.g. Self, Dr. Kumar)" value={newPatient.referredBy} onChange={e=>setNewPatient(p=>({...p,referredBy:e.target.value}))} className="col-span-2 px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-[14px] font-medium outline-none text-slate-900 dark:text-white placeholder:text-slate-400 focus:border-teal-500 focus:ring-1 focus:ring-teal-500 transition-colors" />
            </div>
